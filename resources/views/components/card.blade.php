...
<div data-id="{{ $link->id }}" class="bg-background-secondary min-w-full p-3 rounded-2xl flex flex-wrap items-center gap-3 cursor-grab">
    <div class="flex items-center gap-4">
        ...
        <img src="{{ $link->image }}" alt="{{ $link->title }}" class="object-cover size-12" />
    </div>

    <div class="truncate max-w-50 sm:max-w-xs lg:max-w-md text-content-tertiary">
        <span class="flex items-center gap-2">
            <h3 class="label-large text-content-primary max-w-44 sm:max-w-80 md:max-w-lg truncate">{{ $link->title }}
            </h3>
    ...
    <div class="ml-auto">
        ...
        <button type="button" data-action="edit" data-id="{{ $link->id }}" data-title="{{ $link->title }}"
            ...
            <i class="iconoir-edit text-lg"></i>
        </button>
    </div>
</div>
